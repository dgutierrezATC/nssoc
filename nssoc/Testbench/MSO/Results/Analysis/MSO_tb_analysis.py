...
testbench_results_filenames.append("MSO_tb_input_spikes.txt")
# Then, we add the output spikes to the testbench results file list (index 1)
testbench_results_filenames.append("MSO_tb_output_spikes.txt")
    ...
    with open(testbench_results_file_path) as csv_file:
        ...
        for row in csv_reader:
            ...
            if testbench_results_file_index == 0:
                # First, check if the event is coming from the left or right side
                src = row[0]
                src = int(src)
                ...
                input_spikes_timestamps[src].append(ts)
                input_spikes_addresses[src].append(ad)
                ...
                ch_id = ad / num_neurons
                ch_id = int(ch_id)
                neuron_id = ad % num_neurons

                output_spikes_timestamps[ch_id].append(ts)
                output_spikes_addresses[ch_id].append(neuron_id)
            else:
                print("ERROR!")
