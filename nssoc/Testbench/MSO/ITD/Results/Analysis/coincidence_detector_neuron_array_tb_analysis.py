...
testbench_results_filenames.append("coincidence_detector_neuron_array_tb_input_spikes.txt")
# Then, we add the output spikes to the testbench results file list (index 1)
testbench_results_filenames.append("coincidence_detector_neuron_array_tb_output_spikes.txt")
...
input_spikes_left_timestamps = []
input_spikes_left_addresses = []

input_spikes_right_timestamps = []
input_spikes_right_addresses = []

# Create one list for the output events timestamps and for the addresses
...
fig.suptitle('Coincidence detector neuron testbench results')
...
ax2.plot(output_spikes_timestamps, output_spikes_addresses, 'b|', markersize=15, label='Output')
