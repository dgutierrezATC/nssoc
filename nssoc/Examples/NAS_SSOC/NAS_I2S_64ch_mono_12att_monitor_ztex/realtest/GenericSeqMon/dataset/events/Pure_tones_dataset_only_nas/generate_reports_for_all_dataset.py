...
dataset_path = dataset_path + '\\Dist_300cm\\Config2\\'
...
for subfolder in os.listdir(dataset_path):
    if os.path.isdir(os.path.join(dataset_path, subfolder)):# and (subfolder == '250Hz_aedats' or subfolder == '500Hz_aedats' or subfolder == '750Hz_aedats'):# or subfolder == '250Hz_aedats' or subfolder == '500Hz_aedats' or subfolder == '750Hz_aedats'):
        ...
        for entry in os.listdir(basepath):
            if os.path.isfile(os.path.join(basepath, entry)) and entry.endswith('.aedat'):
                ...
                # Save back the events but only the NAS events
                Savers.save_AEDAT(recording_file_nas, filename.replace('.aedat', ''), settings)
